...
서버에서 설정 파일을 한 번만 만든다.

```bash
cp configs/server.env.example configs/server.env
```

`configs/server.env`에 BERT 모델·SQuAD 자료·단어 목록의 절대 경로, 시험할 초당 요청 수, GPU 번호, 동작 속도와 시작 온도를 적는다. 설정 파일은 Git에 올라가지 않는다.

다음 명령은 환경 준비부터 공통 요청량 선정과 짧은 조건 확인까지 실행한 뒤 멈춘다.

```bash
scripts/run_gpu_pilot.sh prepare
```

출력된 짧은 확인 결과를 검토한 뒤에만 30회 본 실험을 별도로 시작한다.

```bash
scripts/run_gpu_pilot.sh run
```

`run`은 짧은 조건 확인을 통과한 기록이 없으면 본 실험을 거부한다. 본 실험이 중단됐다면 같은 명령으로 완료된 실험을 건너뛰고 이어서 실행한다. 실패한 실험을 새 시도로 다시 실행할 때만 `--rerun-failed`를 추가한다.

```bash
scripts/run_gpu_pilot.sh run --rerun-failed
```

실제 명령을 실행하지 않고 순서만 보려면 `--dry-run`을 추가한다. GPU 동작 속도는 각 단계에서 고정했다가 스크립트 종료 시 원래 설정으로 되돌린다. 시작 온도는 준비·요청량 선정·짧은 확인·본 실험 전에 확인하지만, 30회 내부의 각 실험 직전 냉각을 강제하는 기능은 아직 포함하지 않는다.
